...
psql -c "\copy dgfip_fantoir_temp from 'FANTOIR0417' with csv delimiter '#' quote '>';"
psql -c "
...
create index idx_insee_dgfip_fantoir on dgfip_fantoir (code_insee,id_voie) with (fillfactor=100);
cluster dgfip_fantoir using idx_insee_dgfip_fantoir;
"
