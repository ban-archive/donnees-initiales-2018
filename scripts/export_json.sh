...
echo "\COPY (select format('{\"type\":\"housenumber\", \"cia\": \"\", \"group:ign\":\"%s\" , \"ign\": \"%s\", \"numero\":\"%s\", \"ordinal\":\"%s\" %s %s %s %s}', group_ign, ign, number, ordinal, case when co_postal is not null then ',\"postcode:code\": \"'||co_postal||'\", \"municipality:insee\": \"'||code_insee||'\"' end, case when lb_l5 is not null then ',\"postcode:complement\":\"'||lb_l5||'\"' end, case when ancestor_ign is not null then ',\"ancestor:ign\":\"'||ancestor_ign||'\"' end,',\"attributes\":{\"source_init\":\"'||source_init||'\"}') from housenumber where group_ign is not null and group_fantoir is null and code_insee like '${dep}%') to '${data_path}/${dep}/05_housenumbers.json';" >> commandeTemp.sql
